...
\subsubsection{Model 2: Mixture of sample variation and independent error process}
...
\subsubsection{Model 3: Mixture of sample variation with a mixture of two error processes}
...
\subsubsection{Model 4: Mixture of sample variation and independent error process, simplified to a two-state model}

Model 4 is a simplified version of Model 2. It is motivated by the need to have a likelihood that is directly comparable to the simple 2 state beta-binomial below.

Compared to Model 2, insertion and deletion rates are not jointly analyzed within a single context. Instead one likelihood function is created and minimized separately for each of the two indel types for each contexts.

As an example, when evaluating insertion error rates, all types of insertions (length 1, 2 and 3+), are summed to together to create the alternate allele observation count at each locus, and these are compared to the reference counts. Any observations of deletion alleles are ignored. Because there is only one alt type, we no longer consider an alt-het variant type, and we no longer account for possible error observations mixed into the het or hom variant states. The corresponding pattern is followed when estimating deletions.

\subsubsection{Model 5: Mixture of sample variation with a mixture of two error processes, simplified to a two-state model}
...
\subsubsection{Model 6: Mixture of sample variation with a beta-binomial error process}
...
\begin{equation}
P ( d \vert g_{ref} ) = \frac{B(c(alt)+\alpha,c(ref)+\beta)}{B(\alpha,\beta)}
\end{equation}
