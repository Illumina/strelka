...
\begin{equation*}
P ( d \vert \theta, g_{ref}) = \prod_{y \in Y} r(y)^{c(y)}
\end{equation*}
...
\begin{equation*}
P (d \vert \theta, g_{het}) = 0.5^{c(y_{var}) + c(ref)} \prod_{y \in Y, y \not\in \{y_{var},ref\}} r(y)^{c(y)}
\end{equation*}
...
\begin{equation*}
P (d \vert \theta, g_{hom}) = (1-e_{ref})^{c(y_{var})} {e_{ref}}^{c(ref)} \prod_{y \in Y, y \not\in \{y_{var},ref\}} r(y)^{c(y)}
\end{equation*}

...where $e_{ref}$ is set to a constant value of 0.01. As discussed for the heterozygous likelihood, the simplified representation of error terms such as $e_ref$ here are likely to introduce distortions in indel contexts with higher error rates.
...
\begin{equation*}
P (d \vert \theta, g_{hetalt}) = 0.5^{c(y_{var})+c(y_{var2})} {e_{ref}}^{c(ref)} \prod_{y \in Y, y \not\in \{y_{var},y_{var2},ref\}} r(y)^{c(y)}
\end{equation*}
