...
Alignments in polymorphic regions are prone to errors, often leading to false variant calls. To cope with this problem, state-of-the-art variant callers like GATK and Platypus use local de novo assembly to reconstruct haplotypes in these regions and use them to improve variant calls. Strelka employs a similar haplotyping approach but to keep its superiority in speed it is applied only to relatively short regions ($\leq 250$ by default) where dense variants are expected. We call this simplified haplotyping approach {\em short haplotyping}. 

Strelka's short haplotyping is comprised of 3 steps: (1) detecting short regions with dense variants called {\em active regions}, (2) creating candidate haplotypes, and (3) aligning each haplotype to the reference to discover primitive alleles such as SNVs and short indels.

\subsubsection{Active region detection}
To detect active regions, Strelka first identifies loci likely to have variants called {\em candidate variant loci}. To identify candidate variant loci, for each locus Strelka calculates an alignment depth (the number of alignments overlapping the locus) and a variant count (weighted count of alignments with variants at the locus). Variant counts are updated while reading alignments. A mismatch of an alignment at locus $i$ increases the counter at locus $i$ by 1. An insertion of a sequence between locus $i$ and $i+1$ increases the counters at $i$ and $i+1$ by 4. A deletion of loci $[i,j]$ increases the counters in $[i-1,j]$ by 4. Similar to an indel, a soft-clipped segment also increases the counter. A beginning (or ending) edge soft-clipped segment ending (or starting) at locus $i$ increases the counter at $i$ and $i+1$ (or $i-1$) by 4. After all alignments overlapping a locus are read, Strelka determines whether the locus is a candidate variant locus. A locus with a variant count $c$ and an alignment depth $d$ becomes a {\em candidate variant locus} if (1) $c \geq 0.35 \cdot d$ or (2) $c \geq 9$ and $c \geq 0.35 \cdot d$.

Strelka detects active regions by clustering 2 or more near-by (within 13 bases) candidate variant loci. An important consideration here is that an active region should not start or end within a homopolymer or short tandem repeat (STR) region. Alignments that do not fully span such regions are often incorrect and reconstructing haplotypes containing partial homopolymer or STR sequences is difficult. Therefore Strelka finds {\em anchor loci} that are not candidate variant loci and also not belonging to a homopolymer (of lengths no less than 3) or STR (of repeat unit lengths between 2 and 50) and forces every active region to start and end in anchor loci. Given a cluster of near-by candidate variant loci, the active region is created between the closest anchor loci before and after the first and last candidate variant loci.

\subsubsection{Haplotype generation}
Given an active region of size 250 or smaller, Strelka attempts to generate haplotypes using either simple counting or local de novo assembly. The decision whether to run counting or assembly is made by considering the fraction of reads that fully cover the active region (called covering reads). Out of all reads that overlap with an active region, if 65\% or larger reads are covering reads, Strelka runs counting. The counting procedure is simple. From the covering reads, it first extracts the segments aligned to the region. Then for each unique read segment, it counts the number of reads from which the segment is originated. From the read segments with the read count 3 or larger, it selects top two with the largest counts, which become the candidate haplotypes. 

If the fraction of covering reads is less than 65\%, Strelka tries local de novo assembly using Manta's iterative assembler which uses a de Brujin graph approach (TODO: add a reference and a brief description). The chance that the assembler returns true contigs increases when the contigs do not start or end with repeats. To take advantage of this, given an active region $[i,j]$, Strelka extends the region up to 9 bases upstream and downstream until it encounters a candidate variant locus. Thus the extended active region $[i',j']$ satisfies the following conditions: $i-9 \leq i' \leq i$, $j \leq j' \leq j+9$, and there is no candidate variant locus in $i' \leq pos \leq i-1$ and $j+1 \leq pos \leq j'$. Since $i$ and $j$ are anchor loci, this extension lets the correct contigs returned by the assembler share the same prefix (reference segment at $[i',i]$; denoted by a prefix anchor) and suffix (reference segment at $[j,j']$; denoted by a suffix anchor). 

Local assembly is conducted with all the reads that (fully or partially) overlap with the extended active region, using default parameters of Manta's iterative assembler except for the followings: the minimum word length is set as the sum of the prefix and suffix anchor lengths; and the minimum read coverage is set as 3. For the sake of speed, if the number of reads exceed 1000, assembly is not performed. From all the contigs returned by the assembler, Strelka considers only the contigs with the supporting read count no less than 3 and also including both the prefix and suffix anchors, and selects two contigs with the largest supporting read counts. From these top two contigs, the prefix and suffix anchors are detatched, and the resulting sequences become the candidate haplotypes. 

{\tt TODO: description of sequencer phasing filter}

If the candidate haplotypes are generated by counting, or at least one candidate haplotype other than the reference is generatedy by assembly, the haplotyping is regarded as successful.


\subsubsection{Primitive allele discovery}
If haplotyping is successful, the candidate haplotypes are aligned to the reference segment and from the alignment primitive alleles (SNVs and indels) are identified and marked {\em discovered} in an active region. These discovered primitive alleles are used to improve SNV and indel calling by (1) relaxing the mismatch density filter (described in \ref{sec:BasecallFiltrationFromContext}) and (2) improving indel candidacy (described in \ref{sec:IndelCandidacy}). 
%All the indels that are within an active region where the haplotyping is successful, but not discovered by the haplotype alignment are flagged so that they are not reported in the final output.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{TumorSampleAbsReadPosRankSum}
        \item \texttt{TumorSampleAbsStrandOddsRatio}
...
\begin{itemize}
    ...
    \item \texttt{TumorSampleAbsReadPosRankSum} ~ Absolute value of the \texttt{TumorSampleReadPosRankSum} feature.

    \item \texttt{TumorSampleAbsStrandOddsRatio} ~ Absolute value of the log strand odds ratio of allele counts $log_{10}{\frac{r_{fwd} a_{fwd}} {r_{rev} a_{rev}}}$, given reference $(r_{fwd},r_{rev})$ and non-reference $(a_{fwd},a_{rev})$ confident counts of the tumor sample's tier1 observations (see: ``\nameref{sec:somatic_tiers}'').
