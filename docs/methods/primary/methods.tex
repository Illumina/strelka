...
\begin{enumerate}
    \item Tier1 reads: The most reliable read mappings which are used for all aspects of variant calling.
    \item Tier2 reads: Less reliable reads than tier1, these are used for verification of somatic calls as described further below.
    \item Sub-mapped reads: The least reliable reads, these are retained, together with tier1 and tier2 reads, to enable  variant classification features such as the root mean square mapping quality for all reads covering the variant locus.
\end{enumerate}
