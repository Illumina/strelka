...
Both germline and somatic variants can be described as mixtures of one or more alleles, each occurring at a specific variant allele frequency. When calling variants at a site in a single sample, Strelka evaluates a range of hypotheses, each of which consists of a list of haplotypes along with their hypothesized frequencies in the sample. For instance, valid hypotheses in germline calling correspond to the different possible genotype calls: ``homref'' (a single-component mixture containing only the reference haplotype at 100\% frequency), ``het'' (a two-component mixture containing the reference and one alternative haplotype, each at 50\% frequency), ``homalt'' (a single-component mixture containing a non-reference haplotype at 100\% frequency), and ``hetalt'' (a two-component mixture containing two different non-reference haplotypes, each at 50\% frequency). Hypotheses for somatic calling are described in section \ref{sec:somatic}.

In order to calculate a posterior probability for a hypothesis $H$, we first need to calculate the hypothesis-specific likelihood $P(D \mid H,Q)$ of the observed set of reads $D$ given their corresponding basecall quality scores $Q$. Assuming independence of individual reads $d$ [TODO rephrase after we have a section explaining what is done to relax this assumption], the hypothesis-specific likelihood can be expanded as:
...
Here we are ignoring strand bias, which could potentially be modeled if strand information for each read were included alongside basecall quality information.

We consider that each read is generated from one of the haplotypes that exist in the mixture (the ``generating haplotype''). With the hypothesis $H$ having been specified as a list of haplotypes $h_i$ with corresponding frequencies $f_i$, the likelihood for an individual read can be expressed in terms of per-read likelihoods conditioned on each of the potential generating haplotypes:
\begin{eqnarray}
...
[XXX @csaunders: this description may be oversimplified or just inaccurate - please check and decide on whether we need more detail here XXX]
...
\label{sec:germline}
For each site at which one or more candidate variants have been proposed (all three possible non-reference variants for SNVs or up to two non-reference variants for indels), the valid genotype configurations 
involving those variants are considered as hypotheses. For each hypothesis, we calculate a posterior probability conditioned on the observed data $D$ and $Q$:
...
The hypothesis-specific likelihood $P(D \mid H,Q)$ is calculated as described in section \ref{sec:shared}. The hypothesis priors depend on the corresponding genotype priors $P(G)$ and on the number $N_G$ of hypotheses under consideration for the genotype in question: $P(H) = \frac{P(G)}{N_G}$ [XXX this is true for SNVs, unclear if also true for indels? Specifically, when 2 non-ref candidates are present, are the het and hom prior weights shared between them as they are for SNVs? When only 1 non-ref candidate is present, what happens to the hetalt prior weight (the equation here breaks down)? XXX]. Strelka considers two models for the genotype prior distributions $P(G)$, resulting in two different posterior probabilities. It then reports the more conservative of the two probabilities, which amounts to using the {\bf de novo mutation model} for non-reference variant calls and the {\bf polymorphic site model} for homozygous reference calls.

\subsubsection{De novo mutation model}
This model assumes that mutations arise with probability $\theta_s$ (the Watterson $\theta$ [CITE]), with independent mutation events required to produce hetalt genotypes. [XXX I don't know how hom genotypes acquire their second copy of the mutation under this model? XXX]
...
\subsubsection{Polymorphic site model}
...
The somatic calling model assumes that the samples are from diploid individuals. For both SNVs and indels, the normal genotype state space is the set of unphased diploid genotypes, $G_n \in \{ \texttt{ref}, \texttt{het}, \texttt{hom}\}$, where $\texttt{ref}$, $\texttt{het}$, and $\texttt{hom}$ refer to the normal genotype being reference, heterozygous, and homozygous, respectively. The tumor genotype has two states $G_t \in \{ \texttt{nonsom}, \texttt{som} \}$, where $\texttt{nonsom}$ and $\texttt{som}$ indicate the absence and presence of somatic mutation in the tumor sample, respectively. The method approximates a posterior probability on the joint tumor and normal genotypes.

[XXX any reason hetalt is omitted here? We don't really want to list the normal genotypes in both germline and somatic sections, so if they really are different that complicates things. XXX]
TODO: merge the list of normal genotypes with germline; put in shared section
