...
In the germline model, overlapping indel alleles can be filtered at each locus based on the expected ploidy of each region in each sample. As mentioned above, this process partially replicates the ploidy-based noise filtration that occurs in the haplotype model, but it remains an important step for several reasons, (1) there are variant loci where the haplotype model does not operate for various reasons, for instance because no active region was triggered, assembly failed, the region was too large, etc (2) the overlapping allele filtration step described below is different than that performed in the haplotype model in that it occurs after read realignment and is based on the likelihood of reads to support each of the overlapping alleles in question, rather than being based on simple read counts.

A summary of the germline model overlapping allele filtration procedure is as follows. Given an expected ploidy $p_s$ for the position of interest in each sample $s$, we identify all overlapping indel alleles at that position, rank these by read likelihood support in each sample and retain no more than $p_s$ non-reference alleles in each sample. We then consider all indel alleles at other positions which overlap with those we have retained at the position of interest. This expanded set of indel alleles is again ranked by relative read likelihood support such that no more than $p_s$ non-reference alleles in each sample. The filtration is divided into two phases based on the position of interest because additional heuristics need to be applied in the second phase to resolve alleles with an unknown phasing relationship.

Note that the per-sample ploidy value $p_s$ can be specified by region in each sample as user input to Strelka, and defaults to 2 otherwise. Only ploidy values of 1 or 2 will currently be handled by the methods below.

\paragraph{Resolving overlapping indels indexed to a single position}

Each position in the genome is processed in left to right order. For each position, the first step in indel calling is to find the set $A$ of all indel candidate alleles at that position in addition to the reference allele. Note that the indel position is assigned as the first position following the left-most breakpoint of the left-shifted indel.
...
\paragraph{Resolving overlapping indels indexed to multiple positions}

The next step of the locus allele-selection procedure resolves any additional overlapping indels at other positions, if these exist. To do so, we first define \emph{overlapping indels} as two indels which intersect or are adjacent, such that they could not both be present in the same haplotype. In the procedure below we are constrained to only consider a set of alleles for filtration (and subsequently genotyping), where all alleles in the set are overlapping. For the set of position-specific alleles considered above this property must be true, but additional steps are taken to ensure the overlapping property holds when considering indels at other positions, as described below.
...
$B_x$, a mutually-overlapping subset $A_x$, is then found using the following iterative procedure.
$B_x$ is initially empty. Each allele $a \in A_x$ is tested in order from highest to lowest $t(a)$ value, to see if it overlaps with all alleles in $B_x$. If so, the allele is added to $B_x$, before iterating to the next lowest-scoring allele in $A_x$.
\todo{Konrad: Not sure how to handle the above, I couldn't describe the logic without introducing a kind of "pseudo-code in prose", including a mutating definition of $B_x$, curious about thoughts on cleaner way to convey this?}

As a final step, the overlapping alleles are combined with the original position specific allele set to undergo ploidy-based selection. To do so, per-sample allele support is computed for combined allele set $B = A^{\prime} \cup B_x$, as in equation \ref{eq:perSampleAlleleSupport} above, to find the per-sample allele support $t_s(a), a \in B$. This support is then used to rank alleles in each sample and apply ploidy-based filtration for each sample to produce the final set of alleles to be evaluated at each locus.
