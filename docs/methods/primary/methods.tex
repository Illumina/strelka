...
\begin{equation*}
\begin{array}{lcl}
    x & = & e^{\max(v_i \sum_{0}^{i-1}{((1-z)d_0+zd_1)})}_i \\
    f & = & (1-x)/1-e_i \\
...
\subsubsection{Basecall quality adjustment for mapping error}

% NOTE this needs to move to a somatic/germline shared section

For both germline and somatic SNV models, basecall qualities are reduced to reflect the joint confidence in basecall and mapping accuracy. The adjusted basecall quality used by the SNV calling models is

\begin{equation*}
e_{b^{\prime|}} = (1-e_m)e_b + e_m 3/4
\end{equation*}

where $e_b$ is the basecalling error estimated by the sequencer and $e_m$ is the mapping error. Note that this quality score adjustment is applied prior to any of the dependent error basecall adjustment heuristics used by the germline SNV caller.
