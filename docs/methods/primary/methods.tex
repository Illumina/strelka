...
\section{Small Variant Discovery}

\subsection{Input Read Filtration}

TODO. Outright filtration of unmappped, anomolous, etc. Note additional category of reads that are marked for special processing but not actually filtered (low MAPQ, etc.)
...
Candidate haplotypes are aligned to the reference sequence of the active region. From the alignment, primitive alleles (SNVs and indels) are identified. These primitive alleles are used to improve SNV and indel calling by (1) relaxing the mismatch density filter (described in \ref{BasecallFiltrationFromContext}) and (2) improving indel candidacy (described in \ref{IndelCandidacy}).

\subsection{Basecall filtration from local read context} \label{BasecallFiltrationFromContext}
...
\subsection{Indel Candidacy} \label{IndelCandidacy}
...
By default, prior to a read is being realigned, any soft-cliped segment on the leading and trailing edge is unrolled to assume the segment has a matching alignment to the refernece. This treatment assumes that most soft-clipping reflects an indel which hasn't yet been incorporated into the read's alignment.
...
\section{Small Variant Scoring}

\subsection{Core Likelihood Function}
...
\subsection{Somatic Calling Model and Variant likelihoods}
...
\subsection{Somatic Filtration and Scoring}
...
\begin{itemize}
    ...
    \item \texttt{TumorSampleReadPosRankSum} ~ Z-score of Mann-Whitney U test for reference vs non-reference allele read positions in the tumor sample's tier1 observations.
...
\subsection{Germline Calling Model and Genotype likelihoods}


% snv strand bias note:
% dgt.strand_bias=std::max(lhood_fwd[tgt],lhood_rev[tgt])-lhood[tgt];

\subsection{Germline Variant Phasing}
...
\subsection{Germline Filtration and Scoring}
...
\paragraph{EVS/hard-filter experimental feature descriptions}

Strelka includes a number of other experimental variant classification features. Although not currently used they are described here for reference. This list is not comprehensive.
