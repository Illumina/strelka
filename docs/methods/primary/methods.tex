...
Alignments in polymorphic regions are prone to errors, often leading to false variant calls. To cope with this problem, state-of-the-art germline variant callers like GATK and Platypus use local de novo assembly to reconstruct haplotypes in these regions and use them to improve variant calls. Strelka employs a similar haplotyping approach but to keep its superiority in speed it is applied only to relatively short regions ($\leq 250$ by default) where dense variants are expected. We call this simplified haplotyping approach {\em short haplotyping}. Currently, Strelka utilizes short haplotyping to enhance germline variant calls. In the future, it will also be applied to somatic calling.
...
Local assembly is conducted with all the reads that (fully or partially) overlap with the extended active region, using default parameters of Manta's iterative assembler except for the following: the minimum word length is set as the sum of the prefix and suffix anchor lengths; and the minimum read coverage is set as 3. For the sake of speed, if the number of reads exceed 1000, assembly is not performed. From all the contigs returned by the assembler, Strelka considers only contigs with supporting read counts no less than 3 and also including both the prefix and suffix anchors, and selects the two contigs with the largest supporting read counts. From these top two contigs, the prefix and suffix anchors are detached, and the resulting sequences become the candidate haplotypes. 
