...
The counting process uses the same mapped read scanning and realignment strategy used by the small variant caller {\tt TODO: add reference to somewhere in the document - what exactly does this refer to?}, with the following differences:
 \begin{enumerate}
    ...
    \item Error counts are not collected in regions where the pileup depth is more than 3 times the chromosome mean. (The number of contexts affected by such filtration is recorded so that it is still possible to verify the total number of contexts observed during the counting process.)
 \end{enumerate}
...
We model the counts obtained above as having been generated by two-state mixture processes, with an independent process existing for each context. Every site is modeled as belonging to either the \emph{clean} state (generating essentially no indel errors) or the \emph{noisy} state (generating indel errors independently across reads according to a set of error probabilities to be estimated), with the state-specific models being a simplified version (based on hard observation counts rather than per-read likelihoods) of the full likelihood model presented in sections \ref{sec:germline} and \ref{sec:shared}.
...
\begin{equation*}
P (c \mid n, g_{\text{het}}, \Theta) = m_c 0.5^{c(y_{var}) + c(ref)} \prod_{y \in Y, y \not\in \{y_{var},ref\}} r(y)^{c(y)}.
\end{equation*}
...
\begin{equation*}
P (c \mid n, g_{\text{homalt}}, \Theta) = m_c (1-e_{ref})^{c(y_{var})} {e_{ref}}^{c(ref)} \prod_{y \in Y, y \not\in \{y_{var},ref\}} r(y)^{c(y)}
\end{equation*}
...
\begin{equation*}
P (c \mid n, g_{\text{hetalt}}, \Theta) = m_c 0.5^{c(y_{var})+c(y_{var2})} {e_{ref}}^{c(ref)} \prod_{y \in Y, y \not\in \{y_{var},y_{var2},ref\}} r(y)^{c(y)}.
\end{equation*}
...
TODO: this is an old section, used to be further down in the doc, and is now being removed entirely. Chris, can you confirm that none of what is commented out here is still relevant?}
...
Here we assume that all sites at which candidate haplotypes have been generated belong to the noisy state (see section \ref{sec:indel_error_est}), so that the mixture model formulation used to estimate adaptive germline indel error probabilities is not needed here. We are also assuming independence of individual reads $d$ [TODO rephrase after we have a section explaining what is done to relax this assumption] and ignoring strand bias, which could potentially be modeled if strand information for each read were included alongside basecall quality information.
...
{\tt TODO: below we describe the germline case; add details of what is used for indel error probabilities in somatic.}
The second term in equation \ref{eq:al_lik} is a product of state transition probabilities, using the adaptively estimated noisy-state indel error probabilities $e_i(s,r)$, $e_d(s,r)$, and $e_{\text{ref}}(s,r)$ (see section \ref{sec:indel_error_est}) to penalize alignments whenever the read contains an indel with respect to the generating haplotype:
\begin{equation}
