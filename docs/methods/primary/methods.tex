...
% perspective, but would not typically be included in a theoretical methods descreption.
...
% Environment to enable raggedright for a single paragraph
...
\noindent The indel is considered a candidate variant if $p_{error} < p_{reject}$, where $p_{reject}$ is the rejection threshold for the null hypothesis. The default value $p_{reject}$ is $1\e{-9}$.
...
Following the discovery of candidate alleles (optionally including haplotype modeling), reads are realigned in the context of these candidates. This realignment step has two primary functions, the first is to generate a set of alignments which represent the most likely alignment for the read when constrained to various alternate alleles at each locus. Such alignments are used to assess the read's relative support for different indel alleles as part of downstream indel calling models (more detail on this usage is described in \ref{sec:PerReadLikelihood}). The second function is to find a single \textit{representative} alignment to using for SNV calling. The representative alignment is optimized for a number of criteria such that it is often, but not always, the most probable alignment.

\subsubsection{Alignment search}
The alignment search used during the realignment step is based on the assumption that at least one segment of each input read alignment is already correct, and thus can be used to seed an improved alignment based on a sparse set of intersecting candidate indels. The search proceeds as follows. Each read has a starting alignment provided by some input alignment file, as well as a trial indel set. This trial indel set contains intersecting candidate indels, in addition to any non-candidate indels already present in the starting alignment. If the read does not intersect any candidate indels, no realignment is performed, and the search procedure returns only the input read's starting alignment.

If the read does intersect at least one candidate indel, a set of alignments is built from the starting alignment by recursively toggling indels from the trial set. Each indel is toggled from an existing parent alignment to create two child alignments where the previous alignment is preserved on the left or right side of the toggled indel. New candidate indels are added to the trial set when intersected by child alignments extending beyond the range of its parent. Note that for RNA input, the above procedure will be executed independently for each exon.

This above search process is efficient for the case where a read intersects only one or two candidate indels. Due to exponential complexity with the trial set size, heuristics are used to curtail the alignment search in regions with high candidate indel density. Specifically, the search recursion depth is limited to no more than 5, and will be set lower if required such that the number of enumerated alignments does not exceed 5000.
...
The representative read alignment used for SNV calling is usually the most probable alignment found during the above alignment search, but some allowance is made for an alignment which is nearly the most likely yet more parsimonious. This procedure considers the likelihood of each alignment as described in section \ref{sec:PerReadLikelihood}, and any alignment with a likelihood within a factor of 10 of the most likely is eligible to be representative. Within this eligible pool of alignment candidates, the representative alignment is chosen if it has the (1) fewest indels, (2) fewest non-candidate indels, (3) lowest total insertion length, or (4) lowest total deletion length. The criteria are applied in the order given and the first to produce a representive is used.

If more than one alignment candidate was entered in the eligible representative alignment pool describe above, additional soft-clipping may be added on the edges of the representative alignment such that any conflicting alignments on the edge of the read within the eligible alignment pool are removed. This soft-clipping step is helpful for cases such as an alignment which partially covers an STR, causing alignments to the reference and indel alleles to be equally likely. In such an instance the portion of the alignment extending into the STR is soft-clipped.
...
The unconventional alignment search method used here has favorable runtime when its input assumptions are approximately true, however this comes with several costs. (1) The complexity of the search method is exponential with the number of indels intersecting the alignment. As discussed about this is mitigated with heuristics but remains a problematic feature (2) Assumes that the likelihood of a read for a given haplotype will be approximated by a single most likely alignment -- pairHMM and other alternative approaches could improve on this approximation with a more precise approach (3) The method introduces substantial additional code complexity compared to a more standard method.
...
\begin{enumerate}
    ...
    \item Any ambiguous basecall is filtered out. Any contiguous trailing sequence of ambiguous basecalls on a read cause the end of the read to be effectively trimmed off for variant calling purposes. For the purpose of this filter, an ambiguous basecall is any which indicates more than one base.
    \item The \textit{mismatch density filter} (described below) is applied to a basecall if the local read context is highly diverged form the reference or a local haplotype candidate.
\end{enumerate}
...
The haplotype model used during germline calling enables this filter to be applied more precisely be ignoring mismatches corresponding to local candidate haplotypes. To improve the degree to which this filtration is consistent across of set of jointly analyzed samples, an SNV from a candidate haplotype proposed in any sample can be used in all samples to prevent counting the corresponding base as a mismatch for filtration purposes.
...
Given a haplotype model applied to all calling modes (including somatic), we could eliminate the mismatch density filter and instead apply a term expressing 'how surprising is this haplotype given level X of divergence from the reference?' as part of the genotype stage. On the other hand, if there is a long-term need for such a filter, it would be helpful to eliminate the fixed window size if 41, and instead weigh mismatches as a function of distance from the call position.
...
The site dependency adjustment is both ad-hoc and has a high runtime cost (due ot the sorting step), and thus represents a strong improvement opportunity. The ideal solution would be a model-based fit of site error dependency from real data which could be reflected back in the SNV calling model.
...
% TODO: check technical correctness of above description wrt tier1/tier2 handling, and possibly rephrase
