...
private:
    ...
    void init()
    {
        ...
        RealType p_tmp = (0.5 < _p)? (1 - _p) : _p;
        IntType t_tmp = _t;
        
        m = static_cast<IntType>((t_tmp+1)*p_tmp);

        if(use_inversion()) {
            q_n = pow((1 - p_tmp), static_cast<RealType>(t_tmp));
        } else {
            btrd.r = p_tmp/(1-p_tmp);
            btrd.nr = (t_tmp+1)*btrd.r;
            btrd.npq = t_tmp*p_tmp*(1-p_tmp);
            RealType sqrt_npq = sqrt(btrd.npq);
            btrd.b = 1.15 + 2.53 * sqrt_npq;
            btrd.a = -0.0873 + 0.0248*btrd.b + 0.01*p_tmp;
            btrd.c = t_tmp*p_tmp + 0.5;
    ...
    result_type generate(URNG& urng) const
    {
        ...
        while(true) {
            ...
            if(km <= 15) {
                ...
                RealType t_tmp = -km*km/(2*btrd.npq);
                if(v < t_tmp - rho) return k;
                if(v > t_tmp + rho) continue;
