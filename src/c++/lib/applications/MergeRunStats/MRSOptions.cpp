    ...
    MRSOptions& opt)
{
    ...
    ("output-file", po::value(&opt.outputFilename),
     "merged output stats file (required)")
    ("report-file", po::value(&opt.reportFilename),
     "provide a summary report based on the merged stats");
    ...
    {
        usage(log_os,prog,visible, "Must specify at least 1 input stats file");
    }
    ...
    for (const std::string& statsFilename : opt.statsFilename)
    {
        if (! boost::filesystem::exists(statsFilename))
        {
            ...
            oss << "Stats file does not exist: '" << statsFilename << "'";
        ...
        {
            std::ostringstream oss;
            oss << "Same stats file submitted multiple times: '" << statsFilename << "'";
            usage(log_os,prog,visible,oss.str().c_str());
        }
