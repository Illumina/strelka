    ...
    std::ostream& os)
{
    ...
    double x_all_loghood;
    {
        ...
        double final_dlh;
        error_minfunc errFunc(exportedContextData,isLockTheta);
    ...
    // report:
    {
        ...
        reportIndelErrorRateSet(context, "I", sigInsertTotal, contextData, iter, -x_all_loghood, insertErrorRate, theta, os);

        const double deleteErrorRate(std::exp(normalizedParams[MIN_PARAMS::LN_DELETE_ERROR_RATE]));
        reportIndelErrorRateSet(context, "D", sigDeleteTotal, contextData, iter, -x_all_loghood, deleteErrorRate, theta, os);
    }
}
