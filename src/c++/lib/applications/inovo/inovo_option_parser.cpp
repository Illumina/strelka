...
get_inovo_option_parser(
    inovo_options& opt)
{
    po::options_description inovo_parse_opt_ti("Tumor-sample input");
    inovo_parse_opt_ti.add_options()
    ("tumor-bam-file",
     po::value(&opt.tumor_bam_filename),
     "BAM/CRAM file containing read alignments for the tumor sample (required)")
    ;

    po::options_description inovo_parse_opt_to("Tumor-sample output");
    inovo_parse_opt_to.add_options()
    ...
    po::options_description inovo_parse_opt_sv("Somatic variant-calling");
    ...
    po::options_description inovo_parse_opt_filter("Somatic variant-calling filters");
    inovo_parse_opt_filter.add_options()
    ("inovo-chrom-depth-file", po::value(&opt.sfilter.chrom_depth_file),
    ...
    ("inovo-max-depth-factor", po::value(&opt.sfilter.max_depth_factor)->default_value(opt.sfilter.max_depth_factor),
     "If a chrom depth file is supplied then loci with depth exceeding the mean chromosome depth times this value are filtered")
    ("inovo-skip-header", po::value(&opt.sfilter.is_skip_header)->zero_tokens(),
     "Skip writing header info for the somatic vcf/bed files (usually used to simplify segment concatenation)")
    // snv only:
    ("inovo-snv-max-filtered-basecall-frac", po::value(&opt.sfilter.snv_max_filtered_basecall_frac)->default_value(opt.sfilter.snv_max_filtered_basecall_frac),
     "max filtered call fraction")
    ("inovo-snv-max-spanning-deletion-frac", po::value(&opt.sfilter.snv_max_spanning_deletion_frac)->default_value(opt.sfilter.snv_max_spanning_deletion_frac),
     "max fraction of overlapping deletion reads")
    ("inovo-snv-min-qss-ref", po::value(&opt.sfilter.snv_min_qss_ref)->default_value(opt.sfilter.snv_min_qss_ref),
     "min QSS_ref value")
...
//      "report VQSR metrics in variant records")
    // indel only:
    ("inovo-indel-max-ref-repeat",  po::value(&opt.sfilter.indelMaxRefRepeat)->default_value(opt.sfilter.indelMaxRefRepeat),
     "indels expand/contracting pattterns with greater than this repeat will be filtered out")
    ("inovo-indel-max-int-hpol-length",  po::value(&opt.sfilter.indelMaxIntHpolLength)->default_value(opt.sfilter.indelMaxIntHpolLength),
     "indels are filtered if they overlap 'interrupted homopolymers' greater than this length")
    ("inovo-indel-max-window-filtered-basecall-frac",  po::value(&opt.sfilter.indelMaxWindowFilteredBasecallFrac)->default_value(opt.sfilter.indelMaxWindowFilteredBasecallFrac),
     "indel are filtered if more than this fraction of basecalls are filtered in a 50 base window")
    ("inovo-indel-min-qsi-ref", po::value(&opt.sfilter.sindelQuality_LowerBound)->default_value(opt.sfilter.sindelQuality_LowerBound),
     "min QSI_ref value")
    ;

    po::options_description inovo_parse_opt("Two-sample options");
    inovo_parse_opt.add(inovo_parse_opt_ti).add(inovo_parse_opt_to).add(inovo_parse_opt_sv).add(inovo_parse_opt_filter);
    ...
    visible.add(inovo_parse_opt).add(help_parse_opt);
...
void
finalize_inovo_options(const prog_info& pinfo,
