...
contextLogLhood(
    ...
    const double logNoisyNoIndelAltRate,
    ...
    const double logTheta)
{
    ...
    const double logNoisyNoIndelRefRate(std::log(1-std::exp(logNoisyNoIndelAltRate)));

    const double logCleanLocusRate(std::log(1-std::exp(logNoisyLocusRate)));

    static const double cleanNoIndelAltRate(1e-8);
    static const double logCleanNoIndelAltRate(std::log(cleanNoIndelAltRate));
    static const double logCleanNoIndelRefRate(std::log(1-cleanNoIndelAltRate));
    ...
    for (const auto& obs : observations)
    {
        ...
        const double noisyNoindel(logNoisyNoIndelAltRate*obs.altObservations + logNoisyNoIndelRefRate*obs.refObservations);

        const double cleanNoindel(logCleanNoIndelAltRate*obs.altObservations + logCleanNoIndelRefRate*obs.refObservations);

        /// TODO: generalize log_sum to N values...
        const double mix(log_sum(log_sum(logHomPrior+hom,logHetPrior+het),logNoIndelPrior+log_sum(logNoisyLocusRate+noisyNoindel,logCleanLocusRate+cleanNoindel)));
...
struct error_minfunc : public codemin::minfunc_interface<double>
{
    ...
    virtual double val(const double* in)
    {
        ...
        return -contextLogLhood(_obs,
                ...
                _params[MIN_PARAMS::LN_NOISY_LOCUS_RATE],
        ...
        double* out)
    {
        ...
        out[MIN_PARAMS::LN_NOISY_LOCUS_RATE] =  convert(in[MIN_PARAMS::LN_NOISY_LOCUS_RATE]);
    ...
    const SequenceErrorCounts& counts)
{
    ...
    for (const auto& contextInfo : counts)
    {
        ...
        double x_all_loghood;
        {
            ...
            minParams[MIN_PARAMS::LN_NOISY_LOCUS_RATE] = std::log(1e-2);
        ...
        const double NoisyRate(std::exp(normalizedParams[MIN_PARAMS::LN_NOISY_LOCUS_RATE]));
